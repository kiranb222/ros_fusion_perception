...
import os
import sys
sys.path.append('/home/kiran/dev/sil_ros/src/kitti_ros/scripts/')
import rospy
import std_msgs.msg
...
import pykitti.pykitti_utils as putil
...
if __name__ == "__main__":
   ...
   # print(mode)
   # fps = 10
   # filter_by_camera_angle_ =  True
   # care_objects = ""
   # path = "/home/kiran/catkin_sil/data/2011_09_26/2011_09_26_drive_0005_sync"
    ...
    pose_files.sort()
    poses = []
    for pose in putil.get_oxts_packets_and_poses(pose_files):
        poses.append(pose[1])
    # print len(poses)
    ...
    if calib_imu_to_velo_file:
        calib = putil.read_calib_file(calib_imu_to_velo_file)
    ...
    if use_gt:
        bounding_boxes, tracklet_counter = Preprocessor.KittiPreprocessor.read_label_from_xml(tracklet_file, care_objects)
    ...
    while idx < len(bin_files):
        ...
        ##TODO read data
        pc = Preprocessor.KittiPreprocessor.load_pc_from_bin(bin_path + "/" + bin_files[idx])
        ...
        if filter_by_camera_angle_:
            pc = Preprocessor.KittiPreprocessor.filter_by_camera_angle(pc)
        ...
        if use_gt and idx in bounding_boxes.keys():
            ...
            corners = Preprocessor.KittiPreprocessor.get_boxcorners(places, rotates_z, size)

        KittiPublisher.Publisher.publish_raw_clouds(pub_points, header_, pc)

        if corners is not None:
            KittiPublisher.Publisher.publish_ground_truth_boxes(ground_truth_pub_, header_, places, rotates_z, size)
            # publish_bounding_vertex(pub_vertex, header_, corners.reshape(-1, 3))
            # publish_img_bb(pub_img_bb, header_, corners.reshape(-1, 3))
            KittiPublisher.Publisher.publish_ground_truth_markers(object_marker_pub_, header_, corners.reshape(-1, 3))
            # publish_clusters(pub_clusters, header_, pc, corners.reshape(-1, 3))
        elif use_gt:
            print ("no object in current frame: " + bin_files[idx])
            # publish empty message
            KittiPublisher.Publisher.publish_ground_truth_boxes(ground_truth_pub_, header_, None, None, None)
            KittiPublisher.Publisher.publish_ground_truth_markers(object_marker_pub_, header_, None)


        """
            publish RGB image
        """
        KittiPublisher.Publisher.publish_raw_image(pub_img, header_, image)
        print ("###########")
